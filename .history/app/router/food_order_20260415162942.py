...
def get_info(user_id  : str, order_id : order_id, db : Session = Depends(get_db)):
    """Retrieve an entire data by an ID"""
    info = db.query(User).filter(User.id == user_id).first()
    order = db.query(User).filter(User.id == def get_info(user_id  : str, order_id : order_id, db : Session = Depends(get_db)):
).first()
    if not info:
