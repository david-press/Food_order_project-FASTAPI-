import asyncio 
import uuid
from app.schema.schema import  get_info , order_food , bio_output ,order_update , dashboard_out, order_output 
...
def get_orders(db , user_id : str):
        return db.query(Order).filter(Order.user_id == user_id).all()
    
@router.get("/dashboard/{user_id}", response_model=dashboard_out)
async def get_bio(user_id  : str, db : Session = Depends(get_db)):
